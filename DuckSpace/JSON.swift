...
//  From: https://github.com/owensd/json-swift/blob/master/JSON/JSON.swift
//
...
public enum JSON : Equatable, Printable {
        ...
        if let value : AnyObject = rawValue {
            ...
            case let array as NSArray:
                ...
                for item : AnyObject in array {
                    newArray.append(JSONValue(item))
                }
    ...
    public static func parse(jsonString : String) -> JSON? {
        ...
        return jsonObject != nil ? JSONValue(jsonObject) : nil
    }
    ...
    case .JSONString(let encodedStringWithPrefix):
        if encodedStringWithPrefix.hasPrefix(Encodings.base64.toRaw()) {
            ...
            let bytes = UnsafeBufferPointer<Byte>(start: bytesPointer, count: decoded.length)
            return [Byte](bytes)
        }
